	...
	for (int spineindex = 0; spineindex < spine_Feet_pair.Num(); spineindex++)
	{
		if (InputBoneTransforms.Num() > spine_Feet_pair[spineindex].Spine_Chain_Index)
		{
			...
			for (int feetindex = 0; feetindex < spine_Feet_pair[spineindex].Associated_Feet.Num(); feetindex++)
			{
				...
				//	FTransform ws_inv_spine_const = owning_skel->GetComponentToWorld()*spine_Feet_pair[i].inverted_spine_tf;

				FVector character_direction_vector = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), character_direction_vector_CS);
...
void FAnimNode_DragonSpineSolver::GetAnimatedPoseInfo(FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if ((owning_skel->GetWorld()->IsGameWorld() || Work_Outside_PIE) && test_counter > 4)
	{
		...
		for (int i = 0; i < NumTransforms - 0; i++)
		{
			...
			if (i < (NumTransforms - tip_reduction))
			{
					...
					for (int feet_index = 0; feet_index < spine_Feet_pair[spine_index].Associated_Thighs.Num(); feet_index++)
					{
						...
						if (spine_Feet_pair[spine_index].Associated_Thighs[feet_index].IsValidToEvaluate())
						{
							...
							if (spine_index == 0 && stabilize_pelvis_legs)
							{
								...
								pelvis_slope_stab_alpha = FMath::FInterpTo(pelvis_slope_stab_alpha, stab_value,owning_skel->GetWorld()->DeltaTimeSeconds, Location_Lerp_Speed / 10);
							...
							//	thigh_transform.SetRotation(MeshBases.GetComponentSpaceTransform(spine_Feet_pair[spine_index].Associated_Thighs[feet_index].CachedCompactPoseIndex).GetRotation());
								...
								chest_slope_stab_alpha = FMath::FInterpTo(chest_slope_stab_alpha, stab_value, owning_skel->GetWorld()->DeltaTimeSeconds, Location_Lerp_Speed/10);
				...
				if (spine_AnimatedTransform_pairs.Num() > i)
				{
					...
					for (int j = 0; j < spine_AnimatedTransform_pairs[i].Associated_Feet.Num(); j++)
					{
						if (spine_Feet_pair[i].Associated_Feet.Num() > j)
						{
							...
							spine_AnimatedTransform_pairs[i].Associated_Feet[j].AddToTranslation((UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), spine_Feet_pair[i].feet_trace_offset[j])));

							//	FVector Feet_Offseted_Location = spine_AnimatedTransform_pairs[i].Associated_Feet[j].GetLocation() + (UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), spine_Feet_pair[i].feet_trace_offset[j]));
...
void FAnimNode_DragonSpineSolver::ConditionalDebugDraw(FPrimitiveDrawInterface* PDI, USkeletalMeshComponent* PreviewSkelMeshComp) const
{
	...
	if (DisplayLineTrace && PreviewSkelMeshComp && PreviewSkelMeshComp->GetWorld())
	{
		for (int i = 0; i < TraceStartList.Num(); i++)
		{
			...
			{
				owner_scale = PreviewSkelMeshComp->GetComponentToWorld().GetScale3D().Z * virtual_scale;
			}
...
void FAnimNode_DragonSpineSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	float start_const = (line_trace_upper_height * component_scale);

	float end_const = (line_trace_downward_height * component_scale);
	...
	component_scale = owning_skel->GetComponentTransform().GetScale3D().Z * virtual_scale;
					...
					if (spine_hit_between[i].bBlockingHit)
					{
						...
						else
						{
							...
							//if (is_snake)
							{
								...
								spinal_point = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), spinal_point);

								FVector impact_point_inv = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), spine_hit_between[i].ImpactPoint);
								spinal_point.X = impact_point_inv.X;
								spinal_point.Y = impact_point_inv.Y;
								spinal_point = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), spinal_point);
				...
				for (int32 j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
				{
					...
					if (spine_hit_pairs[i].RV_Feet_Hits[j].bBlockingHit)
					{
						...
						else
						{
							...
							FVector impact_point_inv = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), spine_hit_pairs[i].RV_Feet_Hits[j].ImpactPoint);
							spinal_point.X = impact_point_inv.X;
							spinal_point.Y = impact_point_inv.Y;
							spinal_point = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), spinal_point);
					...
					else
					{
						...
						feet_zeroed_point = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), feet_zeroed_point);
						feet_zeroed_point.Z = 0;
						feet_zeroed_point = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), feet_zeroed_point);


						//feet_zeroed_point.Z = owning_skel->GetComponentToWorld().GetLocation().Z;
			...
			//	if (SolverReferencePose == ERefPosePluginEnum::VE_Animated)
				{
					...
					else
					{
						...
						if (is_snake)
						{
							...
							FVector impact_point_inv = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), spine_hit_pairs[i].Parent_Spine_Hit.ImpactPoint);
							spinal_point.X = impact_point_inv.X;
							spinal_point.Y = impact_point_inv.Y;
							spinal_point = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), spinal_point);
					...
					//	if (every_foot_dont_have_child == false)
					{
						...
						FVector spinal_left_upper = feet_centre_point + character_direction_vector * (line_trace_upper_height * component_scale) + (right_dir * component_scale * virtual_leg_width);
						FVector spinal_right_upper = feet_centre_point + character_direction_vector * (line_trace_upper_height * component_scale) - (right_dir * component_scale * virtual_leg_width);
						FVector spinal_front_upper = feet_centre_point + character_direction_vector * (line_trace_upper_height * component_scale) + (forward_dir * component_scale * virtual_leg_width);
						FVector spinal_back_upper = feet_centre_point + character_direction_vector * (line_trace_upper_height * component_scale) - (forward_dir * component_scale * virtual_leg_width);


						line_trace_func(*owning_skel, spinal_left_upper, feet_centre_point - character_direction_vector * (line_trace_downward_height * 1 * component_scale) + (right_dir * component_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Left_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Left_Hit, FLinearColor::Green, true);
						line_trace_func(*owning_skel, spinal_right_upper, feet_centre_point - character_direction_vector * (line_trace_downward_height * 1 * component_scale) - (right_dir * component_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Right_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Right_Hit, FLinearColor::Green, true);

						line_trace_func(*owning_skel, spinal_front_upper, feet_centre_point - character_direction_vector * (line_trace_downward_height * 1 * component_scale) + (forward_dir * component_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Front_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Front_Hit, FLinearColor::Green, true);
						line_trace_func(*owning_skel, spinal_back_upper, feet_centre_point - character_direction_vector * (line_trace_downward_height * 1 * component_scale) - (forward_dir * component_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Back_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Back_Hit, FLinearColor::Green, true);
						...
						if (spine_hit_pairs[i].Parent_Left_Hit.bBlockingHit)
						{
							...
							else
							{
								...
								FVector impact_point_inv = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), spine_hit_pairs[i].Parent_Left_Hit.ImpactPoint);
								spinal_point.X = impact_point_inv.X;
								spinal_point.Y = impact_point_inv.Y;
								spinal_point = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), spinal_point);
						...
						if (spine_hit_pairs[i].Parent_Right_Hit.bBlockingHit)
						{
							...
							else
							{
								...
								FVector impact_point_inv = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), spine_hit_pairs[i].Parent_Right_Hit.ImpactPoint);
								spinal_point.X = impact_point_inv.X;
								spinal_point.Y = impact_point_inv.Y;
								spinal_point = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), spinal_point);
						...
						if (spine_hit_pairs[i].Parent_Front_Hit.bBlockingHit)
						{
							...
							else
							{
								...
								FVector impact_point_inv = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), spine_hit_pairs[i].Parent_Front_Hit.ImpactPoint);
								spinal_point.X = impact_point_inv.X;
								spinal_point.Y = impact_point_inv.Y;
								spinal_point = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), spinal_point);
						...
						if (spine_hit_pairs[i].Parent_Back_Hit.bBlockingHit)
						{
							...
							else
							{
								...
								FVector impact_point_inv = UKismetMathLibrary::InverseTransformLocation(owning_skel->GetComponentToWorld(), spine_hit_pairs[i].Parent_Back_Hit.ImpactPoint);
								spinal_point.X = impact_point_inv.X;
								spinal_point.Y = impact_point_inv.Y;
								spinal_point = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), spinal_point);
	...
	if (spine_hit_pairs.Num() > 0)
	{
		...
		if (is_snake)
			for (int k = 0; k < spine_hit_between.Num(); k++)
			{
				...
				if ((spine_between_points[k].Z - (owning_skel->GetComponentLocation().Z)) > -Minimum_Feet_Distance * component_scale)
				{
		...
		for (int k = 0; k < spine_hit_pairs.Num(); k++)
		{
			...
			if (use_crosshair_trace_also_for_fail_distance)
			{
				...
				for (int i = 0; i < spine_hit_pairs[k].RV_Feet_Hits.Num(); i++)
				{
					...
					if (FMath::Abs(spine_hit_pairs[k].Parent_Back_Hit.ImpactPoint.Z - (owning_skel->GetComponentLocation().Z)) > Maximum_Feet_Distance * component_scale)
					{
					...
					if (FMath::Abs(spine_hit_pairs[k].Parent_Front_Hit.ImpactPoint.Z - (owning_skel->GetComponentLocation().Z)) > Maximum_Feet_Distance * component_scale)
					{
					...
					if (FMath::Abs(spine_hit_pairs[k].Parent_Left_Hit.ImpactPoint.Z - (owning_skel->GetComponentLocation().Z)) > Maximum_Feet_Distance * component_scale)
					{
					...
					if (FMath::Abs(spine_hit_pairs[k].Parent_Right_Hit.ImpactPoint.Z - (owning_skel->GetComponentLocation().Z)) > Maximum_Feet_Distance * component_scale)
					{
	...
	if (spine_hit_pairs.Num() > 0)
	{
		...
		for (int32 i = 0; i < spine_Feet_pair.Num(); i++)
		{
			...
			for (int32 j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
			{
				...
				spine_Transform_pairs[i].Associated_Feet[j].AddToTranslation((UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), spine_Feet_pair[i].feet_trace_offset[j])));
...
void FAnimNode_DragonSpineSolver::line_trace_func(USkeletalMeshComponent& skelmesh, FVector startpoint, FVector endpoint, FHitResult RV_Ragdoll_Hit, FName bone_text, FName trace_tag, FHitResult& Output, FLinearColor debug_color, bool debug_mode)
{
	...
	if (owning_skel->GetOwner())
	{
		...
		FVector up_dir_ws = owning_skel->GetComponentToWorld().TransformVector(character_direction_vector_CS).GetSafeNormal();

		FHitResult RV_Anti_Checker;


		FVector Origin_Point = startpoint - up_dir_ws * line_trace_upper_height * component_scale;

		if (Use_Anti_Channel)
			UKismetSystemLibrary::LineTraceSingle(owning_skel->GetOwner(), Origin_Point, startpoint, Anti_Trace_Channel, true, ignoreActors, EDrawDebugTrace::None, RV_Anti_Checker, true, debug_color);
		...
		if (RV_Anti_Checker.bBlockingHit && Use_Anti_Channel)
		{
			//	if (RV_Anti_Checker.GetComponent())
			{
				...
				float Origin_Hit_Distance = ((Origin_Point - RV_Anti_Checker.ImpactPoint).Size()) + 1;
				...
				if (response_channel == ECollisionResponse::ECR_Block)
				{
					...
					if (trace_type == EIKTrace_Type_Plugin::ENUM_LineTrace_Type)
						UKismetSystemLibrary::LineTraceSingle(owning_skel->GetOwner(), New_StartPoint, endpoint, Trace_Channel, true, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color);
					else
						if (trace_type == EIKTrace_Type_Plugin::ENUM_SphereTrace_Type)
						{//(startpoint-endpoint).Size()
							UKismetSystemLibrary::SphereTraceSingle(owning_skel->GetOwner(), New_StartPoint, endpoint, Trace_Radius * owner_scale, Trace_Channel, true, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color);
						}
						else
							if (trace_type == EIKTrace_Type_Plugin::ENUM_BoxTrace_Type)
								UKismetSystemLibrary::BoxTraceSingle(owning_skel->GetOwner(), New_StartPoint, endpoint, FVector(1, 1, 0) * Trace_Radius * owner_scale, FRotator(0, 0, 0), Trace_Channel, true, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color);
		...
		else
		{
			...
			if (trace_type == EIKTrace_Type_Plugin::ENUM_LineTrace_Type)
				...
				if (trace_type == EIKTrace_Type_Plugin::ENUM_SphereTrace_Type)
				{//(startpoint-endpoint).Size()
					UKismetSystemLibrary::SphereTraceSingle(owning_skel->GetOwner(), startpoint, endpoint, Trace_Radius * owner_scale, Trace_Channel, true, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color);
				}
				else
					if (trace_type == EIKTrace_Type_Plugin::ENUM_BoxTrace_Type)
...
void FAnimNode_DragonSpineSolver::FABRIK_BodySystem(FComponentSpacePoseContext& Output, FBoneReference TipBone, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	for (int i = 0; i < combined_indices.Num(); i++)
	{
		...
		if (i > 0 && i < combined_indices.Num() - 1)
		{
			...
			FVector new_location = (spine_between_points[i - 1] + character_direction_vector_CS * (spine_between_heights[i - 1]));
			...
			if (spine_hit_between[i - 1].bBlockingHit)
				result.SetLocation(FVector(lerp_data.X, lerp_data.Y, spine_between_points[i - 1].Z + spine_between_heights[i - 1]));
...
void FAnimNode_DragonSpineSolver::Snake_ImpactRotation(int origin_point_index, FTransform& OutputTransform, FCSPose<FCompactPose>& MeshBases)
{
	...
	FVector new_location = owning_skel->GetComponentTransform().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Point + (character_direction_vector_CS * Total_spine_heights[origin_point_index]));
	...
	//test conversion
	lerp_data = owning_skel->GetComponentToWorld().InverseTransformPosition(lerp_data);
	...
	FVector pointing_to_transformpoint = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Point + (character_direction_vector_CS * Total_spine_heights[origin_point_index]));

	FRotator position_Based_rot = final_rot;

	FVector Feet_Mid_Point = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Point);
	...
	if (origin_point_index == 0)
	{
		...
		if (owning_skel->GetOwner())
		{
			...
			if (spine_hit_pairs[origin_point_index].Parent_Front_Hit.bBlockingHit && spine_hit_pairs[origin_point_index].Parent_Back_Hit.bBlockingHit)
				Intensifier_forward = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point).Z) * Pelvis_ForwardRotation_Intensity_INPUT;
			else
			...
			if (spine_hit_pairs[origin_point_index].Parent_Left_Hit.bBlockingHit && spine_hit_pairs[origin_point_index].Parent_Right_Hit.bBlockingHit)
				Intensifier_side = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Point).Z) * Pelvis_SideRotation_Intensity_INPUT * 0.5f;
			else
				Intensifier_side = 0;
			...
			//right_direction = owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetOwner()->GetActorRightVector())*Intensifier_side;

			right_direction = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), FVector::CrossProduct(character_direction_vector_CS, Forward_Direction_Vector)) * Intensifier_side;
		...
		FVector relativePos = (owning_skel->GetComponentToWorld().TransformPosition(pointing_to_transformpoint) - (owning_skel->GetComponentToWorld().TransformPosition(Feet_Mid_Point) + forward_direction)).GetSafeNormal();


		relativePos = (owning_skel->GetComponentToWorld().GetLocation() - DebugEffectorTransform.GetLocation());
	...
	else if (origin_point_index == spine_Transform_pairs.Num() - 1)
	{
		...
		if (owning_skel->GetOwner())
		{
			...
			if (spine_hit_pairs[origin_point_index].Parent_Front_Hit.bBlockingHit && spine_hit_pairs[origin_point_index].Parent_Back_Hit.bBlockingHit)
				forward_intensity = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point).Z) * Chest_ForwardRotation_Intensity_INPUT;
			else
				forward_intensity = 0;
			...
			if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 2 || Use_Fake_Chest_Rotations || spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 0)
			{
				...
				if (spine_hit_pairs[origin_point_index].Parent_Left_Hit.bBlockingHit && spine_hit_pairs[origin_point_index].Parent_Right_Hit.bBlockingHit)
					right_intensity = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Point).Z) * Chest_SideRotation_Intensity_INPUT * 0.5f;
				else
					right_intensity = 0;
			...
			{
				if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 0)
					right_intensity = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_FeetHit_Points[0]).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_FeetHit_Points[1]).Z) * direction * Chest_SideRotation_Intensity_INPUT * -1 * 0.5f;
			...
			right_direction = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), FVector::CrossProduct(character_direction_vector_CS, Forward_Direction_Vector)) * right_intensity;
...
//			FRotator target_rot = FRotator(UKismetMathLibrary::Lerp<float>( spine_RotDifference[origin_point_index].Pitch, spine_RotDifference[origin_point_index].Pitch,0,adapt), final_rot.Yaw, spine_RotDifference[origin_point_index].Roll).GetNormalized();
	...
	//	lerp_data = owning_skel->GetComponentToWorld().TransformPosition(lerp_data_temp);
	...
	if (spine_hit_pairs.Num() > origin_point_index - 1)
	{
		...
		FVector right_dir_vector = FVector::CrossProduct(forward_dir_forward, character_direction_vector_CS);
		...
		//		FVector pitch_based_location = RotateAroundPoint(spine_position, right_dir_vector, spine_hit_pairs[origin_point_index].Parent_Spine_Point, OutputTransform.Rotator().Pitch);
				...
				*/
				//float component_scale = owning_skel->GetComponentScale().Z;
		...
		//	if (Z_Offset == 10000000000)
		{
			if (spine_hit_pairs[origin_point_index].Parent_Spine_Hit.bBlockingHit)
				Z_Offset = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Point);
			else
		...
		FVector lerp_data_temp = lerp_data;
		lerp_data = owning_skel->GetComponentToWorld().TransformPosition(lerp_data_temp);

		FVector Z_Offset_temp = Z_Offset;
		Z_Offset = owning_skel->GetComponentToWorld().TransformPosition(Z_Offset_temp);
		...
		location_reset = owning_skel->GetComponentToWorld().TransformPosition(location_reset);

		//	final_transformation = owning_skel->GetComponentToWorld()*final_transformation;
...
//				if ((  (FVector(lerp_data.X, lerp_data.Y, Z_Offset + Total_spine_heights[origin_point_index]* component_scale)) - lerp_data).Size() > 10000*component_scale)
//					OutputTransform.SetLocation(FVector(lerp_data.X, lerp_data.Y, Z_Offset + Total_spine_heights[origin_point_index]*component_scale));
//				OutputTransform.SetLocation(lerp_data + Z_Offset + character_direction_vector*Total_spine_heights[origin_point_index] * component_scale);
//				else
				{
	...
	if (spine_hit_pairs.Num() > origin_point_index - 1)
	{
		...
		if (atleast_one_hit && spine_hit_pairs[origin_point_index].Parent_Spine_Hit.bBlockingHit)
		{
			...
			*/
			...
			//final_bone_rotation.Yaw = owning_skel->GetComponentToWorld().Rotator().Yaw;
		...
		//	final_rot = FRotator((YZ_rot.Pitch), final_bone_rotation.Yaw, XZ_rot.Roll);
			...
			*/
			...
			FVector parent_spine_hit_CS = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Point);
			...
			//if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 2)
			{
				...
				if (dragon_bone_data.FeetBones.Num() == 0)
				{
					...
					inv_transform.SetLocation(owning_skel->GetComponentToWorld().InverseTransformPosition(inv_transform.GetLocation()));
...

				{
					...
					//	Z_Offset = owning_skel->GetComponentToWorld().TransformPosition(Z_Offset);
				...
				//	Z_Offset = FVector(Z_Offset.X, Z_Offset.Y, FMath::Clamp(Z_Offset.Z, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z - Maximum_Formated_Dip_Height, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z + Maximum_Formated_Dip_Height));


				if (origin_point_index == 0)
					Z_Offset = FVector(Z_Offset.X, Z_Offset.Y, FMath::Clamp(Z_Offset.Z, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z - Maximum_Formated_Dip_Height, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z + Maximum_Formated_Dip_Height));
				else
					Z_Offset = FVector(Z_Offset.X, Z_Offset.Y, FMath::Clamp(Z_Offset.Z, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z - Maximum_Formated_Dip_Height_Chest, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z + Maximum_Formated_Dip_Height_Chest));
			...
			if (spine_hit_pairs[opposite_index].RV_Feet_Hits.Num() == 2)
			{
				...
				if (spine_hit_pairs[opposite_index].RV_FeetHit_Points[0].Z > spine_hit_pairs[opposite_index].RV_FeetHit_Points[1].Z)
					feet_difference_offset_opp = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[opposite_index].RV_FeetHit_Points[1]).Z;
				else
					feet_difference_offset_opp = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[opposite_index].RV_FeetHit_Points[0]).Z;
			...
			if(spine_hit_pairs[opposite_index].RV_Feet_Hits.Num() == 0)
				Feet_Opposite_Mid_Point = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint);
			else
			 Feet_Opposite_Mid_Point = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[opposite_index].Parent_Spine_Point);
			...
			if (origin_point_index == 0)
				rotation_offset = Pelvis_Rotation_Offset;
			else if (origin_point_index == spine_Feet_pair.Num() - 1)
				rotation_offset = Chest_Rotation_Offset;
		...
		//	FVector feet_mid_world = owning_skel->GetComponentToWorld().TransformPosition( Feet_Mid_Point);

		//	FVector feet_opp_world = owning_skel->GetComponentToWorld().TransformPosition(Feet_Opposite_Mid_Point);



			float AC_Cross_Value = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point).Z);

			if (flip_forward_and_right)
			{

				AC_Cross_Value = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Point).Z);
			...
			if (atleast_one_hit)
			{
				...
				if (origin_point_index == 0)
				{
					across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point).Z) * Pelvis_Slope_Detection_Strength, 0, 1);

					if (flip_forward_and_right)
					{
						across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Point).Z) * Pelvis_Slope_Detection_Strength, 0, 1);
				...
				else
				{
					...
					across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point).Z) * Chest_Slope_Detection_Strength, 0, 1);

					if (flip_forward_and_right)
					{
						across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Point).Z) * Chest_Slope_Detection_Strength, 0, 1);
				...
				*/
				...
				else
				{
					...
					if (Use_Fake_Chest_Rotations == false)
					{
						...
						opposite_root_transform = owning_skel->GetComponentToWorld().InverseTransformPosition(opposite_root_transform);

						FVector relative_root_offset = opposite_root_transform - owning_skel->GetComponentToWorld().InverseTransformPosition(RootEffectorTransform.GetLocation());
				...
				if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 2)
				{
					...
					FVector feet_0_impact_cs = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_FeetHit_Points[0]);

					FVector feet_1_impact_cs = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_FeetHit_Points[1]);

					//owning_skel->GetComponentToWorld().InverseTransformPosition

					if ((origin_point_index == 0 && (Use_Fake_Pelvis_Rotations || is_this_biped) ) || (origin_point_index > 0 && Use_Fake_Chest_Rotations))
					{
						feet_0_impact_cs = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Point);
						feet_1_impact_cs = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Point);
					}
				...
				spine_LocDifference[origin_point_index] = character_direction_vector_CS * (owning_skel->GetComponentTransform().GetLocation().Z - mesh_transformy.GetLocation().Z);
			...
			if (origin_point_index == 0)
			{
				...
				if (owning_skel->GetOwner())
				{
							...
							/*	if (flip_forward_and_right)
								{
									...
									Intensifier_forward = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Point).Z)* Pelvis_SideRotation_Intensity_INPUT*0.5f;
					...
					//	forward_direction = owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetOwner()->GetActorForwardVector()) * Intensifier_forward;
					...
					//forward_direction = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), Forward_Direction_Vector) * Intensifier_forward;
					...
					if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 2 || (Use_Fake_Pelvis_Rotations || is_this_biped) || spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 0)
					{
						...
						if (spine_hit_pairs[origin_point_index].Parent_Left_Hit.bBlockingHit && spine_hit_pairs[origin_point_index].Parent_Right_Hit.bBlockingHit)
						{
							/*if (flip_forward_and_right)
							{
								Intensifier_side = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point).Z)* Pelvis_SideRotation_Intensity_INPUT*0.5f;
							}
							else
							*/
							{
					...
					else
					{
						...
						Intensifier_side = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_FeetHit_Points[0]).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_FeetHit_Points[1]).Z) * direction * Pelvis_SideRotation_Intensity_INPUT * -1 * 0.5f;
				...
				if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 2 || (Use_Fake_Pelvis_Rotations || is_this_biped) || spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 0)
				{
					...
					position_Based_rot.Pitch = (Intensifier_side * -1) + rotation_offset;
					position_Based_rot.Roll = Intensifier_forward * -1;
				...
				else
				{
					...
					if (flip_forward_and_right)
					{
						position_Based_rot.Pitch = (Intensifier_forward * 1) + rotation_offset;
						position_Based_rot.Roll = Intensifier_side * 1;
					}
					else
					{
			...
			//	position_Based_rot.Roll = Roll_look_rot.Roll;
				...
				else
				{
					...
					if (flip_forward_and_right)
					{
						position_Based_rot.Pitch = (forward_intensity * 1) + rotation_offset;
						position_Based_rot.Roll = right_intensity * 1;
					}
					else
					{
						position_Based_rot.Pitch = (right_intensity * -1) + rotation_offset;
						position_Based_rot.Roll = forward_intensity * -1;
					}
				}
...
//			FRotator target_rot = FRotator(UKismetMathLibrary::Lerp<float>( spine_RotDifference[origin_point_index].Pitch, spine_RotDifference[origin_point_index].Pitch,0,adapt), final_rot.Yaw, spine_RotDifference[origin_point_index].Roll).GetNormalized();
		...
		//OG	FRotator target_rot = FRotator(UKismetMathLibrary::Lerp(0, spine_RotDifference[origin_point_index].Pitch, Adaptive_Alpha), final_rot.Yaw, UKismetMathLibrary::Lerp(0, spine_RotDifference[origin_point_index].Roll,Adaptive_Alpha) ).GetNormalized();
				...
				//if (LineTraceInitialized == true)
			{
				...
				FVector right_dir_vector = FVector::CrossProduct(forward_dir_forward, owning_skel->GetComponentToWorld().TransformVector(character_direction_vector_CS));
				...
				final_transformation.SetLocation(owning_skel->GetComponentToWorld().TransformPosition(final_transformation.GetLocation()));
				...
				FVector pitch_based_location = RotateAroundPoint(spine_position, forward_dir_forward * -1, spine_hit_pairs[origin_point_index].Parent_Spine_Point, OutputTransform.Rotator().Pitch);
				...
				FVector offset_worldspace = owning_skel->GetComponentToWorld().TransformVector(character_direction_vector_CS * (base_offset));
				...
				if (origin_point_index == 0)
					final_transformation.SetLocation(FVector(final_transformation.GetLocation().X, final_transformation.GetLocation().Y, FMath::Clamp(final_transformation.GetLocation().Z, (owning_skel->GetComponentToWorld().TransformPosition(lerp_data)).Z - Maximum_Formated_Dip_Height, owning_skel->GetComponentToWorld().TransformPosition(lerp_data).Z + Maximum_Formated_Dip_Height)));
				else
					final_transformation.SetLocation(FVector(final_transformation.GetLocation().X, final_transformation.GetLocation().Y, FMath::Clamp(final_transformation.GetLocation().Z, (owning_skel->GetComponentToWorld().TransformPosition(lerp_data)).Z - Maximum_Formated_Dip_Height_Chest, owning_skel->GetComponentToWorld().TransformPosition(lerp_data).Z + Maximum_Formated_Dip_Height_Chest)));
...
DragonSpineOutput FAnimNode_DragonSpineSolver::DragonSpineProcessor(FTransform& EffectorTransform, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	else*/
	{
		...
		//	else
		{
			...
			{
				//1 - FMath::Exp(-Smooth_Factor * owning_skel->GetWorld()->GetDeltaSeconds()), Formatted_Location_Lerp* 0.05f)
				//Max_Range_Limit_Lerp = FMath::Lerp<float>(Max_Range_Limit_Lerp, Max_Range_Limit, owning_skel->GetWorld()->DeltaTimeSeconds*5);
...
DragonSpineOutput FAnimNode_DragonSpineSolver::DragonSpineProcessor_Direct(FTransform& EffectorTransform, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	// Start with Root Bone
	{
		...
		FVector Bone_World = owning_skel->GetComponentToWorld().TransformPosition(BoneCSTransform.GetLocation());

		FVector Bone_World_Root = owning_skel->GetComponentToWorld().TransformPosition(FVector(BoneCSTransform.GetLocation().X, BoneCSTransform.GetLocation().Y,0));
