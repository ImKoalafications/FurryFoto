...
//	if(owning_skel->GetWorld()->IsGameWorld())
	{
		...
		for (int i = 0; i < HandIK_Transforms.Num(); i++)
		{
			...
			Lerp_HandIK_Transforms[i].Transform = UKismetMathLibrary::TInterpTo(Lerp_HandIK_Transforms[i].Transform, HandIK_Transforms[i].Transform,owning_skel->GetWorld()->DeltaTimeSeconds,Hand_Interpolation_Speed);
			else
...
void FAnimNode_DragonAimSolver::UpdateInternal(const FAnimationUpdateContext & Context)
{
	...
	if (turn_on_solver)
		toggle_alpha = UKismetMathLibrary::FInterpTo(toggle_alpha, 1, owning_skel->GetWorld()->GetDeltaSeconds(), Toggle_Interpolation_Speed);
	else
		toggle_alpha = UKismetMathLibrary::FInterpTo(toggle_alpha, 0, owning_skel->GetWorld()->GetDeltaSeconds(), Toggle_Interpolation_Speed);
...
//	FVector TTS_Ref_Pos = UKismetMathLibrary::TransformLocation(owning_skel->GetComponentToWorld(), Debug_LookAtLocation.GetLocation() );


	FVector TTS_Ref_Pos = (Debug_LookAtLocation * owning_skel->GetComponentToWorld()).GetLocation();
	...
	Zeroed_Hit.SetLocation((Zeroed_Hit * owning_skel->GetComponentToWorld()).GetLocation());
...
bool FAnimNode_DragonAimSolver::IsValidToEvaluate(const USkeleton * Skeleton, const FBoneContainer & RequiredBones)
{
	...
	for (int i = 0; i < dragon_input_data.FeetBones.Num(); i++)
	{
		...
		if (foot_bone_ref.IsValidToEvaluate(RequiredBones) == false || knee_bone_ref.IsValidToEvaluate(RequiredBones) == false || thigh_bone_ref.IsValidToEvaluate(RequiredBones) == false)
			feet_is_true = false;
...
void FAnimNode_DragonAimSolver::InitializeBoneReferences(FBoneContainer & RequiredBones)
{
	...
	Lerp_HandIK_Transforms.AddDefaulted(Aiming_Hand_Limbs.Num()*4);
...
//	if (toggle_alpha > 0.01f)
	{
		...
		{
			LookAtLocation_Temp = Debug_LookAtLocation;
			Body_LookTarget = Debug_LookAtLocation;
		...
		if (enable_solver)
		{
			...
			if (Adaptive_Terrain_Tail)
			{
				...
				//LookAtLocation_Temp = Debug_LookAtLocation*owning_skel->GetComponentToWorld();
		...
		//	LookAtLocation_Temp = LookAtLocation_Saved;
			...
			*/
			...
			if (Aggregate_Hand_Body && Use_Separate_Targets)
			{
				FVector Head_World = owning_skel->GetComponentToWorld().TransformPosition( Head_Orig_Transform.GetLocation() );
			...
			FAnimationRuntime::ConvertBoneSpaceTransformToCS(owning_skel->GetComponentToWorld(), MeshBases, LookHeadTransform, EndSplineBone.GetCompactPoseIndex(*SavedBoneContainer), EBoneControlSpace::BCS_WorldSpace);
			...
			UDragonIK_Library::Evaluate_ConsecutiveBoneRotations(Output, owning_skel, spine_Feet_pair, Look_Bending_Curve, StartSplineBone, EndSplineBone, Lookat_Radius, Inner_Body_Clamp, LookAtLocation_Temp, LookatAxis_Temp, Lookat_Radius, Vertical_Dip_Treshold, Downward_Dip_Multiplier, Inverted_Dip_Multiplier, Side_Move_Multiplier, Side_Down_Multiplier, false, FTransform::Identity, Look_Multiplier_Curve, Up_Rot_Clamp, Use_Natural_Method, Head_Use_Separate_Clamp, Lookat_Clamp, LookHeadTransform, Override_Head_Rotation, HeadTransforms);
			...
			//	HeadTransforms[HeadTransforms.Num() - 1] = Lookat_Rotation_Rotator;
				...
				//	HeadTransforms[HeadTransforms.Num() - 1].Transform.SetRotation(LookAtLocation_Temp.GetRotation() * Head_Orig_Transform.GetRotation());
					...
					HeadTransforms[HeadTransforms.Num() - 1].Transform.SetRotation( (owning_skel->GetComponentTransform().GetRotation().Inverse()* LookAtLocation_Temp.GetRotation()) * Head_Orig_Transform.GetRotation());
					...
					if (Main_Arm_Index > -1 && Main_Arm_Index < Hand_Array.Num())
					{
						...
						if (Actual_Shoulder_Array[Main_Arm_Index].IsValidToEvaluate() && Elbow_Array[Main_Arm_Index].IsValidToEvaluate() && Hand_Array[Main_Arm_Index].IsValidToEvaluate())
						{
							...
							if (Connector_Index.GetInt() > 0)
							{
								...
								FVector Target_CS_Position = owning_skel->GetComponentToWorld().InverseTransformPosition(Arm_LookTarget);


								/*
								if (Aiming_Hand_Limbs[Main_Arm_Index].override_limits)
									Target_CS_Position = (ClampRotateVector(Arm_LookTarget, owning_skel->GetComponentToWorld().TransformVector(LookAt_Axis), owning_skel->GetComponentToWorld().TransformPosition(Head_Orig_Transform.GetLocation()), Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_V_Angle.X, Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_V_Angle.Y, Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_H_Angle.X, Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_H_Angle.Y, Use_Natural_Method));
								else
									Target_CS_Position = (ClampRotateVector(Arm_LookTarget, owning_skel->GetComponentToWorld().TransformVector(LookAt_Axis), owning_skel->GetComponentToWorld().TransformPosition(Head_Orig_Transform.GetLocation()), Verticle_Range_Angles.X, Verticle_Range_Angles.Y, Horizontal_Range_Angles.X, Horizontal_Range_Angles.Y, Use_Natural_Method));
									*/
							...
							//	Debug_Hand_Locations[Main_Arm_Index].SetLocation(Hand_Transform_Default.GetLocation());
								...
								if (!ignore_elbow_modification)
								{
									...
									if (reach_instead)
									{
										...
										Offseted_Hand_Transform.SetLocation(ClampRotateVector(owning_skel->GetComponentToWorld().TransformPosition(Offseted_Hand_Transform.GetLocation()), owning_skel->GetComponentToWorld().TransformVector(LookAt_Axis), owning_skel->GetComponentToWorld().TransformPosition(Shoulder_Transform_Output.GetLocation()), Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_V_Angle.X, Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_V_Angle.Y, Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_H_Angle.X, Aiming_Hand_Limbs[Main_Arm_Index].Max_Arm_H_Angle.Y, Use_Natural_Method));

										Offseted_Hand_Transform.SetLocation(owning_skel->GetComponentToWorld().InverseTransformPosition(Offseted_Hand_Transform.GetLocation()));
									...
									if (Arm_TargetLocation_Overrides.Arm_TargetLocation_Overrides.Num() > Main_Arm_Index)
									{
										...
										{
											FQuat Offseted_Rotation_Value = owning_skel->GetComponentToWorld().GetRotation().Inverse() * Debug_Hand_Locations[Main_Arm_Index].GetRotation();
									...
									UDragonIK_Library::Evaluate_TwoBoneIK_Direct_Modified(Output, owning_skel, Hand_Array[Main_Arm_Index], Elbow_Array[Main_Arm_Index], Actual_Shoulder_Array[Main_Arm_Index], Hand_Transform, Shoulder_Transform_Output, Knee_Transform_Output, Hand_Transform_Output, FVector(0, 0, 0), FVector(0, 0, 0), Inv_Common_Spine, Common_Spine_Transform, Limb_Rotation_Offset, Aiming_Hand_Limbs[Main_Arm_Index], Individual_Leg_Clamp, FTransform::Identity, Aiming_Hand_Limbs[Main_Arm_Index].Elbow_Pole_Offset, Override_Hand_Rotation, Knee_Transform_Default, LookAt_Axis, Reference_Constant_Forward_Temp, main_arm_LSA, nsew_pole_method, up_arm_twist_technique, Upward_Axis, Use_Separate_Targets, reach_instead, HandIK_Transforms);
				...
				for (int limb_index = 0; limb_index < Aiming_Hand_Limbs.Num(); limb_index++)
				{
					...
					if ( (Actual_Shoulder_Array[limb_index].IsValidToEvaluate() && Elbow_Array[limb_index].IsValidToEvaluate() && Hand_Array[limb_index].IsValidToEvaluate()) && (limb_index != Main_Arm_Index || Main_Arm_Index < 0 || reach_instead) )
					{
						...
						if (Connector_Index.GetInt() > 0)
						{
							...
							Target_CS_Position = owning_skel->GetComponentToWorld().InverseTransformPosition(Target_CS_Position);
							...
							if (!ignore_elbow_modification)
							{
								...
								else
								{
									...
									Offseted_Hand_Transform.SetLocation(Shoulder_Transform_Output.GetLocation() + (Arm_Hand_Vector * FMath::Clamp<float>(Arm_Length, Arm_Length * FMath::Abs(Aiming_Hand_Limbs[limb_index].Minimum_Extension), Arm_Length * FMath::Abs(Aiming_Hand_Limbs[limb_index].Maximum_Extension))));
								...
								if (Arm_TargetLocation_Overrides.Arm_TargetLocation_Overrides.Num() > limb_index)
								{
									...
									if (Override_Hand_Rotation)
									{
										FQuat Offseted_Rotation_Value = owning_skel->GetComponentToWorld().GetRotation().Inverse() * Debug_Hand_Locations[limb_index].GetRotation();

										//FQuat Offseted_Rotation_Value = Arm_TargetLocation_Overrides.Arm_TargetLocation_Overrides[limb_index].Overrided_Arm_Transform.GetRotation();
								...
								else
								{
									if (Override_Hand_Rotation)
									{
										FQuat Offseted_Rotation_Value = owning_skel->GetComponentToWorld().GetRotation().Inverse() * LookAtLocation.GetRotation();
								...
								UDragonIK_Library::Evaluate_TwoBoneIK_Direct_Modified(Output, owning_skel, Hand_Array[limb_index], Elbow_Array[limb_index], Actual_Shoulder_Array[limb_index], Offseted_Hand_Transform, Shoulder_Transform_Output, Knee_Transform_Output, Hand_Transform_Output, FVector(0, 0, 0), FVector(0, 0, 0), Inv_Common_Spine, Common_Spine_Transform, Limb_Rotation_Offset, Aiming_Hand_Limbs[limb_index], Individual_Leg_Clamp, Main_Relative_Transform, Elbow_Pole_Transform.GetLocation(), Override_Hand_Rotation, Knee_Transform_Default, LookAt_Axis, Reference_Constant_Forward_Temp, Last_Shoulder_Angles[limb_index], nsew_pole_method, up_arm_twist_technique,Upward_Axis, Use_Separate_Targets, reach_instead, HandIK_Transforms);
