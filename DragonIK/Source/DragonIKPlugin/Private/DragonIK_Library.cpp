...
void UDragonIK_Library::Evaluate_TwoBoneIK_Spinal_Modified(FComponentSpacePoseContext& Output, USkeletalMeshComponent* owning_skel, FBoneReference FeetBone, FBoneReference KneeBone, FBoneReference ThighBone, FTransform ThighTransform, FTransform Shoulder_Trans, FTransform Knee_Trans, FTransform Hand_Trans, FVector JointLocation, FVector Knee_Pole_Offset, bool Use_NSEW_Poles, bool Use_Up_Arm_Twist, TArray<FBoneTransform>& OutBoneTransforms)
{
...
//	UpperLimbCSTransform.SetRotation(Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex).GetRotation());
//	LowerLimbCSTransform.SetRotation(Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex).GetRotation());

//	UpperLimbCSTransform = (Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex));
//	LowerLimbCSTransform = (Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex));
	...
	UDragonIK_Library::Solve_Modified_Direct_TwoBoneIK_4(owning_skel->GetComponentTransform(), 0,UpperLimbCSTransform, LowerLimbCSTransform, EndBoneCSTransform, JointTargetPos, DesiredThighPos, false, 1, 1, Use_Up_Arm_Twist);
...
void UDragonIK_Library::Evaluate_TwoBoneIK_Direct_Modified(FComponentSpacePoseContext& Output, USkeletalMeshComponent* owning_skel, FBoneReference FeetBone, FBoneReference KneeBone, FBoneReference ThighBone, FTransform ThighTransform, FTransform Shoulder_Trans, FTransform Knee_Trans, FTransform Hand_Trans, FVector JointLocation, FVector Knee_Pole_Offset, FTransform transform_offset, FTransform Common_Spine_Modified_Transform, FRotator Limb_Rotation_Offset, FDragonData_ArmsData& HandData, float Hand_Clamp_Value, FTransform Extra_Hand_Offset, FVector Elbow_Pole_Offset, bool override_hand_rotation, FTransform Knee_Transform_Default,FVector LookAt_Axis_Input,FVector Reference_Constant_Forward_Axis_Input, float& last_shoulder_angle,bool Use_NSEW_Poles,bool up_arm_twist_technique, FVector Up_Vector_Val,bool Separate_Arms_Logic_Used,bool is_reach_mode, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	bool shoulder_twist_available = false;


	if (HandData.Clavicle_Bone.IsValidToEvaluate() && Separate_Arms_Logic_Used)
	{
	
		shoulder_twist_available = true;
		CachedClavicleIndex = HandData.Clavicle_Bone.GetCompactPoseIndex(BoneContainer);

		clavicle_transform = Output.Pose.GetComponentSpaceTransform(CachedClavicleIndex)*transform_offset;
	...
	//	FVector clavicle_direction = (clavicle_transform.GetLocation() - Shoulder_Trans.GetLocation()).GetSafeNormal();
		...
		FVector Clavicle_Effector_Direction = (clavicle_transform.GetLocation() - ThighTransform.GetLocation()).GetSafeNormal();
...
//	FTransform EndBoneCSTransform = Output.Pose.GetComponentSpaceTransform(IKBoneCompactPoseIndex) * transform_offset;
	...
	Pole_Orig_Transform.SetLocation(Elbow_Pole_Offset);
...

	{
		...
		FQuat Pole_Rot_Diff = FQuat::FindBetweenVectors(  (ThighTransform.GetLocation()- UpperLimbCSTransform.GetLocation()), (EffectorTransform.GetLocation() - UpperLimbCSTransform.GetLocation()) );
	...
	FQuat Forward_Rotation_Difference = FQuat::FindBetweenNormals(LookAt_Axis_Input, Reference_Constant_Forward_Axis_Input);
	...
	Pole_Transform.SetLocation(Elbow_Pole_Offset);


	FTransform Second_Pole_Transform = FTransform::Identity;
	Second_Pole_Transform.SetLocation(Knee_Pole_Offset);
...
//OLD1	JointTargetTransform.SetLocation(JointTargetTransform.GetLocation() + Pole_Transform.GetLocation());
	...
	if (is_reach_mode)
	{
		...
		if (Use_NSEW_Poles)
		{
			FVector forward_dir = LookAt_Axis_Input;

			FVector right_dir = FVector::CrossProduct(LookAt_Axis_Input, Up_Vector_Val);
...
//	FAnimationRuntime::ConvertBoneSpaceTransformToCS(Output.AnimInstanceProxy->GetComponentTransform(), Output.Pose, BS_OG_EndBoneCSTransform, IKBoneCompactPoseIndex, EBoneControlSpace::BCS_ComponentSpace);
	...
	UDragonIK_Library::Solve_Modified_Direct_TwoBoneIK_4(owning_skel->GetComponentTransform(), wrist_offset,UpperLimbCSTransform, LowerLimbCSTransform, EndBoneCSTransform, JointTargetPos, DesiredPos, false, 1, 1,up_arm_twist_technique);
	...
	//if (!bAllowTwist)
	{
		...
		FAxis TwistAxis = FAxis();
		TwistAxis.Axis = FVector(1,0,0);
		TwistAxis.bInLocalSpace = true;
	...
	*/
	...
	// Update transform for lower bone.
	{
		...
		//	LowerLimbCSTransform.SetRotation((ThighTransform.GetRotation() * UpperLimbCSTransform.GetRotation()));
			...
			//LowerLimbCSTransform.SetRotation(FRotator(Knee_Trans.Rotator().Pitch, LowerLimbCSTransform.Rotator().Yaw, LowerLimbCSTransform.Rotator().Roll).Quaternion());
	...
	// Update transform for end bone.
	{
		...
		//	EndBoneCSTransform.SetRotation(Default_Rot);
			...
			input_arm_rot = GetHandYaw(true,HandData,owning_skel->GetComponentToWorld(), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), Output.Pose.GetComponentSpaceTransform(IKBoneCompactPoseIndex), LowerLimbCSTransform, EndBoneCSTransform, Unmodifed_Hand,Up_Vector_Val);
		...
		if (rotate_everything)
		{
			...
			else
			{
				...
				FTransform EndBoneCSTransform_Output = LookAt_Processor(EndBoneCSTransform_Always, EndBoneCSTransform_Always.GetLocation(), Far_Target, LookAt_Axis, Hand_Clamp_Value, FRotator(0,0,0),true, 1,1);
...
//	BS_LowerLimbTransform.SetRotation(UKismetMathLibrary::InverseTransformRotation(Reference_Parent, BS_LowerLimbTransform.Rotator()).Quaternion());



//	FRotator limb_rotator = BS_LowerLimbTransform.Rotator();
	//limb_rotator.Pitch = 0;
	//limb_rotator.Roll = 0;
	//limb_rotator.Yaw = 0;

//	BS_LowerLimbTransform.SetRotation(limb_rotator.Quaternion());

//	BS_LowerLimbTransform.SetRotation(UKismetMathLibrary::TransformRotation(Reference_Parent, BS_LowerLimbTransform.Rotator()).Quaternion() );




	//FAnimationRuntime::ConvertBoneSpaceTransformToCS(Output.AnimInstanceProxy->GetComponentTransform(), Output.Pose, BS_EndBoneCSTransform, IKBoneCompactPoseIndex, EBoneControlSpace::BCS_WorldSpace);



	//BS_LowerLimbTransform.SetRotation( (Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex)* BS_EndBoneCSTransform).GetRotation() );
	


	
//	FAnimationRuntime::ConvertBoneSpaceTransformToCS(Output.AnimInstanceProxy->GetComponentTransform(), Output.Pose, BS_EndBoneCSTransform, IKBoneCompactPoseIndex, EBoneControlSpace::BCS_WorldSpace);
//	FAnimationRuntime::ConvertBoneSpaceTransformToCS(Output.AnimInstanceProxy->GetComponentTransform(), Output.Pose, BS_LowerLimbTransform, CachedLowerLimbIndex, EBoneControlSpace::BCS_WorldSpace);

//	BS_LowerLimbTransform.SetRotation(UKismetMathLibrary::FindLookAtRotation(BS_EndBoneCSTransform.GetLocation(), BS_LowerLimbTransform.GetLocation()).Quaternion());





//	FAnimationRuntime::ConvertCSTransformToBoneSpace(Output.AnimInstanceProxy->GetComponentTransform(), Output.Pose, BS_LowerLimbTransform, CachedLowerLimbIndex, EBoneControlSpace::BCS_WorldSpace);
	

	//Output.AnimInstanceProxy->GetComponentTransform(), Output.Pose, CSEffectorTransform, RootBone_Input.GetCompactPoseIndex(BoneContainer), EBoneControlSpace::BCS_WorldSpace

	//LowerLimbCSTransform.SetRotation( (LowerLimbLocalTransform.Inverse() * EndBoneCSTransform).GetRotation() );
	//BS_LowerLimbTransform.SetRotation(ThighTransform.GetRotation()* LowerLimbCSTransform.GetRotation());


//	FRotator input_arm_rot = GetHandYaw(owning_skel->GetComponentToWorld(),Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), Output.Pose.GetComponentSpaceTransform(IKBoneCompactPoseIndex), LowerLimbCSTransform, EndBoneCSTransform);	
...
//	input_arm_rot.Roll = UKismetMathLibrary::Lerp(0, ( input_arm_rot.Roll), roll_alpha);

	/*
	float Pitch_Yaw_Max = FMath::Max<float>( FMath::Abs(input_arm_rot.Yaw), FMath::Abs(input_arm_rot.Pitch));
	Pitch_Yaw_Max = Pitch_Yaw_Max / 90;

	input_arm_rot.Roll = FMath::Lerp<float>(input_arm_rot.Roll,0, Pitch_Yaw_Max);
	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " input_arm_rot " + input_arm_rot.ToString());
	*/


	BS_LowerLimbTransform = SetArmYaw(HandData.invert_lower_twist,HandData.is_this_right_hand,input_arm_rot.Roll,owning_skel->GetComponentToWorld(), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), Output.Pose.GetComponentSpaceTransform(IKBoneCompactPoseIndex), LowerLimbCSTransform, EndBoneCSTransform);
	...
	/*
	input_arm_rot.Roll = FMath::ClampAngle(input_arm_rot.Roll, start_clamp, end_clamp);

	if (input_arm_rot.Roll > 0)
	{
		input_arm_rot.Roll = input_arm_rot.Roll - start_clamp;
	}
	else
	{
		input_arm_rot.Roll = input_arm_rot.Roll - end_clamp;
	}
	*/
	...
	//GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " HandData.last_forarm_angle " + FString::SanitizeFloat(HandData.last_forarm_angle));
	...
	BS_LowerLimbTransform = SetArmYaw(HandData.invert_lower_twist, HandData.is_this_right_hand, HandData.last_forarm_angle, owning_skel->GetComponentToWorld(), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), Output.Pose.GetComponentSpaceTransform(IKBoneCompactPoseIndex), LowerLimbCSTransform, EndBoneCSTransform);


	//input_arm_rot.Roll = input_arm_rot.Roll > 0 ? forearm_angle : -forearm_angle;
...
//	BS_UpperLimbTransform = SetArmYaw(HandData.is_this_right_hand, input_arm_rot.Roll, owning_skel->GetComponentToWorld(), Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), UpperLimbCSTransform, LowerLimbCSTransform);



	//float forearm_angle = UKismetMathLibrary::RadiansToDegrees( (BS_LowerLimbTransform.GetRotation().Inverse() * LowerLimbCSTransform.GetRotation()).GetAngle());

//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " forearm_angle " + FString::SanitizeFloat(forearm_angle));
	...
	FRotator Forearm_Input_Rot = GetHandYaw(false,HandData,owning_skel->GetComponentToWorld(), Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), BS_UpperLimbTransform, BS_LowerLimbTransform, BS_LowerLimbTransform, Up_Vector_Val);


	
	//Forearm_Input_Rot.Roll = FMath::UnwindDegrees(Forearm_Input_Rot.Roll);
	...
	//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " TRUE " + FString::SanitizeFloat(Forearm_Input_Rot.Roll));
	...
	//else
	//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " FALSE " + FString::SanitizeFloat(last_shoulder_angle));

	

//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " Forearm_Input_Rot.Roll " + FString::SanitizeFloat(Forearm_Input_Rot.Roll)+"  original_rot : "+ FString::SanitizeFloat(original_rot));

	/*
	if (shoulder_twist_available)
	{



		//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " Forearm_Input_Rot "+ Forearm_Input_Rot.ToString());


		FTransform Shoulder_Difference = Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex).Inverse() * UpperLimbCSTransform;

		FTransform ShoulderTwistBone = Output.Pose.GetComponentSpaceTransform(CachedClavicleIndex) * Shoulder_Difference;

		//ShoulderTwistBone = SetArmYaw(HandData.is_this_right_hand, last_shoulder_angle * Arm_Angle, owning_skel->GetComponentToWorld(), Output.Pose.GetComponentSpaceTransform(CachedUpperTwistLimbIndex), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), ShoulderTwistBone, BS_LowerLimbTransform);

		BS_UpperLimbTransform = SetArmYaw(HandData.invert_upper_twist, HandData.is_this_right_hand, last_shoulder_angle * Arm_Angle, owning_skel->GetComponentToWorld(), Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), BS_UpperLimbTransform, BS_LowerLimbTransform);


		//OutBoneTransforms.Add(FBoneTransform(CachedUpperTwistLimbIndex, ShoulderTwistBone));
	...
	//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " last_shoulder_angle * Arm_Angle " + FString::SanitizeFloat(last_shoulder_angle * Arm_Angle));
...
///////////
	...
	FVector Arm_Hand_Cross_Dir = FVector::CrossProduct(Arm_Hand_Dir, Up_Vector_CS);
	...
	float direction_sign = FMath::Sign<float>(FVector::CrossProduct(Hand_Point_Dir, Arm_Hand_Cross_Dir).Z);
...
//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " DiffAngle_Forward " + FString::SanitizeFloat(DiffAngle_Forward));
	...
	float side_multiplier = FMath::Min<float>(Angular_Offset1, Angular_Offset2);
...
void UDragonIK_Library::Solve_Modified_Direct_TwoBoneIK_4(FTransform comp_transform, float wrist_rotation, FTransform& InOutRootTransform, FTransform& InOutJointTransform, FTransform& InOutEndTransform, const FVector& JointTarget, const FVector& Effector, bool bAllowStretching, float StartStretchRatio, float MaxStretchScale,bool up_arm_twist_tech)
{
	...
	Solve_Modified_Direct_TwoBoneIK_3(comp_transform,wrist_rotation,InOutRootTransform, InOutJointTransform, InOutEndTransform, JointTarget, Effector, UpperLimbLength, LowerLimbLength, bAllowStretching, StartStretchRatio, MaxStretchScale, up_arm_twist_tech);
}

void UDragonIK_Library::Solve_Modified_Direct_TwoBoneIK_3(FTransform comp_transform, float wrist_rotation, FTransform& InOutRootTransform, FTransform& InOutJointTransform, FTransform& InOutEndTransform, const FVector& JointTarget, const FVector& Effector, float UpperLimbLength, float LowerLimbLength, bool bAllowStretching, float StartStretchRatio, float MaxStretchScale, bool up_arm_twist_tech)
{
...
void UDragonIK_Library::Solve_Modified_Direct_TwoBoneIK_2(const FVector& RootPos, const FVector& JointPos, const FVector& EndPos, const FVector& JointTarget, const FVector& Effector, FVector& OutJointPos, FVector& OutEndPos, bool bAllowStretching, float StartStretchRatio, float MaxStretchScale)
{
		...
		if (Slop > Precision)
		{
			...
			for(IterationCount = 0; IterationCount < MaxIteration; IterationCount ++)
			{
				...
				for (int32 LinkIndex = TipBoneLinkIndex - 1; LinkIndex > 0; LinkIndex--)
				{
					...
					float index_ratio = (float)LinkIndex / (float)TipBoneLinkIndex;

					FVector clamped_dir = UDragonIK_Library::LookAt_Fabrik_Processor(ChainTransform, ChildLink.Position, FVector(0,-1,0), index_ratio*90 );
				...
				*/
				...
				for (int32 LinkIndex = 1; LinkIndex < TipBoneLinkIndex; LinkIndex++)
				{
					...
					FVector clamped_dir = UDragonIK_Library::LookAt_Fabrik_Processor(ChainTransform, ParentLink.Position, FVector(0, -1, 0), index_ratio*90 );
...
FQuat UDragonIK_Library::GetLimitedRotation(TArray<FCCDIK_Modified_ChainLink>& InOutChain,FQuat OriginalBoneRotation, int index, FQuat q,bool& changed)
{
	...
	FQuat parentRotation = GetParentSolverRotation(InOutChain,index);
	FQuat localRotation = parentRotation.Inverse() * q;

//	FQuat limitedLocalRotation = InOutChain[index].rotationLimit.GetLimitedLocalRotation(localRotation,changed);

	FQuat limitedLocalRotation = UDragonIK_Library::GetLimitedLocalRotation(OriginalBoneRotation,localRotation, changed);
...
void UDragonIK_Library::LimitFabrikBoneRotations(TArray<FCCDIK_Modified_ChainLink>& InOutChain)
{
	...
	*/
	...
	for (int chain_index = InOutChain.Num() - 1; chain_index > 1; chain_index--)
	{
		...
		FVector current_bone_pos = InOutChain[chain_index].Position;
		FVector parent_bone_pos = InOutChain[chain_index - 1].Position;

		FVector original_dir = current_bone_pos - parent_bone_pos;
		FVector original_dir_normalized = original_dir.GetSafeNormal();
		float original_len = original_dir.Size();
		...
		float index_ratio = ((float)chain_index / (float)InOutChain.Num());
